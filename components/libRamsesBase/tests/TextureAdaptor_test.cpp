	...
	void checkTextureFormats(raco::core::SEditorObject texture, const std::map<ramses::ETextureFormat, raco::core::ErrorLevel>& formats) {
		for (const auto& [format, level] : formats) {
			...
			if (level == raco::core::ErrorLevel::NONE) {
				...
				ASSERT_TRUE(commandInterface.errors().hasError({texture, &raco::user_types::Texture::textureFormat_}));
				ASSERT_EQ(commandInterface.errors().getError({texture, &raco::user_types::Texture::textureFormat_}).level(), level);
			}
		}
	}
...
TEST_F(TextureAdaptorFixture, textureFormat8BitPalette) {
	...
	checkTextureFormats(texture, {{ramses::ETextureFormat::R8, raco::core::ErrorLevel::WARNING},
...
TEST_F(TextureAdaptorFixture, textureFormatChangeValidToInvalid) {
	...
	dispatch();
	ASSERT_TRUE(commandInterface.errors().hasError({texture, &raco::user_types::Texture::textureFormat_}));
	ASSERT_EQ(commandInterface.errors().getError({texture, &raco::user_types::Texture::textureFormat_}).level(), raco::core::ErrorLevel::ERROR);

	// R8
	commandInterface.set({texture, &raco::user_types::Texture::textureFormat_}, static_cast<int>(ramses::ETextureFormat::R8));
	dispatch();
	ASSERT_TRUE(commandInterface.errors().hasError({texture, &raco::user_types::Texture::textureFormat_}));
	ASSERT_EQ(commandInterface.errors().getError({texture, &raco::user_types::Texture::textureFormat_}).level(), raco::core::ErrorLevel::WARNING);
